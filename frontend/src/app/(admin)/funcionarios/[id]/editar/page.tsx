...
export default function EditarFuncionario() {
  ...
  async function handleUpdateEmployee(event: FormEvent<HTMLFormElement>) {
    ...
    const { name, cpf, email, birthDate, sex, ongEmployeeId } = employeeData;
  ...
  useEffect(() => {
    async function getEmployee() {
      const { data } = await api.get(`/employees/findById/${id}`);

      const { name, email, cpf, birthDate, sex, ongEmployeeId } = data;

      setEmployeeData({
        name,
        email,
        cpf,
        birthDate: new Date(birthDate).toLocaleString('pt-BR', {
          timeZone: 'UTC',
        }),
        sex,
        ongEmployeeId,
  ...
  return (
    <div className='max-w-3xl w-full mx-auto'>
      <h1 className='text-3xl my-8 font-bold'>Edite a conta do funcionário</h1>

      <main className='mb-8'>
        <form className='flex flex-col gap-10' onSubmit={handleUpdateEmployee}>
